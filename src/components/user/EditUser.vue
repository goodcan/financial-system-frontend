<template>
  <div>
    ...
    <el-form :model="user" ref="user" :rules="rules" label-width="100px" class="create-form-size">
      ...
      <el-form-item v-if="!isAdmin" label="权限：">
          <span v-if="pmsListName.length !== 0" v-for="each in pmsListName">
            <el-tag style="margin-right: 5px">{{each}}</el-tag>
          </span>
          <span v-if="pmsListName.length === 0">
                没有任何权限
          </span>
      </el-form-item>
      <el-form-item>
      ...
      init() {
        axios.post('/api/editUserInitData', {
          ...
          if (res.code === 1) {
            ...
            this.pmsListName = res.result.user.pmsListName;
